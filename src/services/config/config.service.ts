...
export class Config extends ServiceMixin<Config>() {
  ...
  private _getViteConfig<TConfig>(importMetaEnv?: ImportMetaEnv): object | undefined {
    ...
    if (Object.keys(filteredConfig).length > 0) {
      debug(`Vite config found: ${JSON.stringify(filteredConfig)}`, Config.constructor.name);
  ...
  private _getInjectedConfig(): object | undefined {
    ...
    if (injectedConfig) {
      debug(`Injected config found: ${JSON.stringify(injectedConfig)}`, Config.constructor.name);
