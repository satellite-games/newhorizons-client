 ...
 * import { error } from '@firefly/web-core/services/logger';
 *
 * error("Hello World!");
 *
...
export class Logger extends ServiceMixin<Logger>() {
  ...
  static debug(message: string, context?: string): void {
    const msg = Logger.instance.createAndPushMessage(message, 'debug', context);
    if (Logger.level !== 'debug') return;
    console.debug(Logger.instance.createMessageString(msg));
  }
  ...
  static info(message: string, context?: string): void {
    const msg = Logger.instance.createAndPushMessage(message, 'info', context);
    if (Logger.level === 'warn' || Logger.level === 'error') return;
    console.info(Logger.instance.createMessageString(msg));
  }
  ...
  static warn(message: string, context?: string): void {
    const msg = Logger.instance.createAndPushMessage(message, 'warn', context);
    if (Logger.level === 'error') return;
    console.warn(Logger.instance.createMessageString(msg));
  }
  ...
  static error(message: string, context?: string): void {
    const msg = Logger.instance.createAndPushMessage(message, 'error', context);
    console.error(Logger.instance.createMessageString(msg));
  }
